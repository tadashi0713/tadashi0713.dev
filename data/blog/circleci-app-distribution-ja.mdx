...
しかし、ビルドしたアプリケーション(主にデバック版)を iOS/Android デバイスにインストール・検証するためには CircleCIだけで完結することができませんでした。
...
iOS/Android プラットフォームへビルドする CircleCI の CI/CD パイプラインの中に、このOrbが提供している `upload_android`, `upload_ios` を追加します。
...
このステップ自体には別途特別なセットアップは必要ありません。

実行すると、CircleCI の Job の Steps の中に `Generate QR Code(You can see QR Code in this step)` があり、この Step をクリックするとコンソール画面に QR コードが表示されます。

![QR Code](https://res.cloudinary.com/tadashi0713/image/upload/v1704777131/cci-app-distribution-qr_duw5j2.png)

これらの QR コードを iOS / Android デバイスで読み取ると配布ページが表示され、対象のアプリケーションをインストールすることが可能です。
...
CircleCI App Distribution では `comment_github_pr` を用いることによって、この配信情報を GitHub の Pull Request にコメントすることができます。
...
事前に [Slack の Token(SLACK_ACCESS_TOKEN)](https://github.com/CircleCI-Public/slack-orb/wiki/Setup)し、CircleCIに保存する必要があります。
...
- 簡単にセットアップ・利用することができる
  ...
  - GitHub Pull Request へのコメント(`comment_github_pr`) と Slack への通知(`notify_slack`) も数行追加するだけで実現することが可能
- セキュアに利用することが可能
...
### GitHub Pull Request へのコメントと Slack への通知を利用する場合

GitHub Pull Request へのコメント(`comment_github_pr`) と Slack への通知(`notify_slack`)を利用する場合には、同じJobの前のステップで `upload_android` もしくは `upload_ios` を実行してください。
...
`upload_android` を実行すると、まず対象のバイナリ(.apk)からアプリ情報(アプリ名、リリースバージョンなど)を解析します。

Android SDK Build-Tools の1つである[AAPT2](https://developer.android.com/studio/command-line/aapt2) を使って解析します。

取得したアプリ情報は、まず HTML ファイルに書き出し、バイナリ(.apk)と共に CircleCI Artifact にアップロードします。

また、GitHub Pull Request へのコメント(`comment_github_pr`) と Slack への通知(`notify_slack`)でこの情報を利用するために、環境変数に保存しておきます。

最後に、アップロードしたHTMLファイルのURLを元にQRCodeを生成し(qrencoder)、CircleCIのジョブコンソールや GitHub Pull Request に表示させます。

`QRコード→配布ページ(HTML)→バイナリ(.apk)をダウンロード・インストール`
...
そのため、マニフェストファイル(.plist)・HTMLファイルの2つをアップロードし、
