...
This `notify_flaky_tests` workflow does not trigger until `notify_flaky_tests` parameter is passed.
...
I added the parameter `notify_flaky_tests` so that this scheduled pipeline can trigger the `notify_flaky_tests` job.
