---
title: セルフホスト(self-managed)版 GitLab と CircleCI クラウドを IP アドレスベースでセキュアに接続する方法
date: '2023-05-08'
tags: ['circleci', 'gitlab', 'japanese']
draft: false
summary: この記事では、CircleCI が提供する IP アドレス範囲機能を利用して、セルフホスト(self-managed)版 GitLab と CircleCI クラウドを IP アドレスベースでセキュアに接続する方法について紹介します。
images:
  [
    'https://res.cloudinary.com/tadashi0713/image/upload/v1682243393/tadashi0713.dev/gitlab-ip-ranges.jpg',
  ]
---

![セルフホスト(self-managed)版 GitLab と CircleCI クラウドを IP アドレスベースでセキュアに接続する方法](https://res.cloudinary.com/tadashi0713/image/upload/v1682243393/tadashi0713.dev/gitlab-ip-ranges.jpg)

この記事では、CircleCI が提供する IP アドレス範囲機能を利用して、セルフホスト(self-managed)版 GitLab と CircleCI クラウドを IP アドレスベースでセキュアに接続する方法について紹介します。

<TOCInline toc={props.toc} />

# CircleCI クラウドのセルフホスト(self-managed)版 GitLab サポート

CircleCI クラウドはこの度、セルフホスト(self-managed)版 GitLab をサポートするようになりました。

[CircleCI に GitLab が対応 | CircleCI](https://circleci.com/ja/blog/announcing-gitlab-support/)

これによって、セルフホスト(self-managed)版 GitLab を利用している開発者でも、CircleCI の高性能な CI/CD 環境を利用することができます。

![CircleCIとGitLabCIの比較](https://res.cloudinary.com/tadashi0713/image/upload/v1683680041/tadashi0713.dev/gitlabci-vs-circleci-ja.png)

しかし、セルフホスト(self-managed)版 GitLab と CircleCI クラウドを接続するためには以下の制約があります。

> self-managed インスタンスはパブリックインターネットからアクセス可能である必要があります。ファイアウォールの内側に配置している場合は使用できません。

しかし、特に企業でセルフホスト(self-managed)版 GitLab を利用している場合には、やみくもにパプリックインターネットからアクセスできる状況にしたくないと思われます。

今回はこの問題を解決するために、CircleCI が提供する IP アドレス範囲機能を利用して、セルフホスト(self-managed)版 GitLab と CircleCI クラウドを IP アドレスベースでセキュアに接続する方法について紹介します。

# CircleCI の IP アドレス範囲機能

通常、CircleCI を含めた SaaS 型の CI/CD ツールでは

CircleCI が提供する IP アドレス範囲機能を利用することによって、CircleCI で使用される 30 個の IP アドレス (一覧はこちら) のいずれかに、送信トラフィックがルーティングされるようになります。

IP アドレスベースのファイアウォールで環境を守りながら、CircleCI との通信だけを許可することができます。

[IP アドレスの範囲機能で、より安心できるセキュリティを確保 | CircleCI](https://circleci.com/ja/blog/ip-ranges-better-security/)

今回はこの IP アドレス範囲機能を利用して、以下の図のようなアーキテクチャーを構成します。

![セルフホスト(self-managed)版 GitLab × IP アドレス範囲アーキテクチャー](https://res.cloudinary.com/tadashi0713/image/upload/v1683296290/tadashi0713.dev/gitlab-ip-ranges-architecture-ja.png)

# 事前準備

- セルフホスト(self-managed)版 GitLab

  - Community Editon, Enterprise Edition どちらでも可能です。
  - 今回は [GitLab Helm Chart](https://docs.gitlab.com/charts/) を利用してインストールしています。

- CircleCI クラウド
  - IP アドレス範囲機能を利用するため、Performance プランまたは Scale プランである必要があります。

# セルフホスト(self-managed)版 GitLab で CircleCI の IP アドレスを許可する

まずはセルフホスト(self-managed)版 GitLab 　において、以下の IP アドレスを許可するように設定します。

- IP アドレス範囲

- コアサービ

今回は GitLab Helm Chart を利用してインストールしているため、同時にインストールされている NGINX Ingress Controller の `loadBalancerSourceRanges` を使って IP アドレスを許可します。

```yaml
nginx-ingress:
  controller:
    service:
      loadBalancerSourceRanges: [
        # Core
        18.214.70.5/32,
        52.20.166.242/32,
        18.214.156.84/32,
        54.236.156.101/32,
        ...

        # IP Ranges
        3.228.39.90/32,
        18.213.67.41/32,
        34.194.94.201/32,
        34.194.144.202/32,
        34.197.6.234/32,
        ...
```

# プロジェクトのセットアップ

次に、IP アドレスを許可したセルフホスト(self-managed)版 GitLab と CircleCI を接続した上で、プロジェクトのセットアップを行います。

以下のドキュメントにある「GITLAB SELF-MANAGED」に従ってセットアップを行なってください。

[GitLab との連携 - CircleCI](https://circleci.com/docs/ja/gitlab-integration/)

# IP アドレス範囲機能を有効にしたパイプラインを作成する

プロジェクトのセットアップが完了したら、IP アドレス範囲機能を有効にしたパイプラインを作成していきます。

**すべてのジョブ**に対して IP アドレス範囲を有効にするフラグ `circleci_ip_ranges: true` を追加してください。

```yaml
jobs:
  unit_test:
    circleci_ip_ranges: true # IP アドレス範囲機能を有効
    docker:
      - image: cimg/node:18.14.0
    steps:
      - checkout
      - node/install-packages
      - run: npm run test:ci
```

設定ファイルが正しく

CircleCI の UI だけではなく、GitLab の Merge Request などの UI からステータスを確認することができます。

# Docker ビルドを利用したい場合(kaniko)

現在 IP アドレス範囲機能は Docker Executor のみの対応になっています。

そのため、Machine Executor(Linux VM など)を利用して、Docker ビルドを行うことはできません。

しかし Docker Executor 内であっても、[kaniko](https://github.com/GoogleContainerTools/kaniko) を利用することによって DinD(Docker)

DinD Docker ビルド、そして各レポジトリ(Docker Hub, Amazon ECR, Azure Container Registry)

以下は IP アドレス範囲機能を有効にした上で、kaniko を利用して Docker ビルド・DockerHub への Push を行なっているジョブの例です。

```yaml
kaniko-build-push-docker-hub:
  circleci_ip_ranges: true
  environment:
    DOCKER_REGISTRY: docker.io
  docker:
    - image: gcr.io/kaniko-project/executor:debug
      entrypoint: ''
  steps:
    - checkout
    - run:
        name: add Docker Hub credentials
        command: |
          mkdir -p /kaniko/.docker
          ./config.sh
          mv config.json /kaniko/.docker
    - run:
        name: Build and Push image
        command: |
          /kaniko/executor \
              --context "$(pwd)" \
              --dockerfile "$(pwd)/Dockerfile" \
              --destination "${DOCKER_REGISTRY}/tadashi0713/gitlab-kaniko:${CIRCLE_SHA1}"
```

詳細は以下のドキュメントを参考にしてください。

[How-to: build and push Docker images with Kaniko - Tips, Tricks and Hacks - CircleCI Discuss](https://discuss.circleci.com/t/how-to-build-and-push-docker-images-with-kaniko/44923)

# macOS を利用したい場合

CircleCI では

[IP アドレス範囲が公開されています](https://circleci.com/docs/ja/ip-ranges/#circleci-macos-cloud)

こちらの IP アドレス範囲を先ほどと同じく
(こちらは IP アドレス範囲機能を有効にしていただくても利用できます)

```yaml
nginx-ingress:
  controller:
    service:
      loadBalancerSourceRanges: [
        # macOS
        162.252.208.0/24,
        162.252.209.0/24,
        192.206.63.0/24,
        162.221.90.0/24,
        38.39.177.0/24,
        ...
```

これによって、

# 注意点

- 現在 IP アドレス範囲機能は、[Performance プランおよび Scale プラン](https://circleci.com/ja/pricing/)で利用できます。

- 現在、IP アドレス範囲機能を使用できるのは、Docker Executor (`remote_docker` を除く) のみです。

- IP アドレスの範囲機能の利用には、この機能を有効にしたジョブで使用されるデータ 1GB につき、450 クレジットがかかります。

# まとめ

この記事では、CircleCI が提供する IP アドレス範囲機能を利用して、セルフホスト(self-managed)版 GitLab と CircleCI クラウドを IP アドレスベースでセキュアに接続する方法について紹介しました。
