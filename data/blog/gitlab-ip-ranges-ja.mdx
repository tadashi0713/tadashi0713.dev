...
- IP アドレス範囲機能で割り当てられる IP アドレス

- コアサービス(ジョブのトリガーや CircleCI と GitLab 間でのユーザーに関する情報の交換などに使用されるサービス)用の IP アドレス
...
そのため、以下の理由で通常の方法で Docker イメージのビルド・プッシュを行うことはできません。
...
CircleCI クラウドの活用によって、制限のない並列数・ビルドスピード、そして開発者が求める高度な CI/CD 機能を手に入れることができます。

今回紹介したソリューションも含めて、ぜひ検討していただければと思います。
